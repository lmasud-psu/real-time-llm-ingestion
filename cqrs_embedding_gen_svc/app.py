...
This service implements a CQRS (Command Query Responsibility Segregation) pattern
to watch for changes in the PostgreSQL database and generate embeddings for new text entries.

Architecture:
- Polls PostgreSQL for new text entries
- Generates embeddings using SentenceTransformer model
- Stores embeddings in a separate table with pgvector extension
"""
...
class EmbeddingProcessor:
    ...
    def store_embedding(self, message_id: str, text_content: str, embedding: np.ndarray):
        ...
        try:
            ...
            logger.info(f"Stored embedding for message ID: {message_id}")
    ...
    def polling_loop(self):
        ...
        logger.info("Starting embedding processor polling loop")
...
def health_check():
    ...
    return jsonify({'status': 'healthy', 'service': 'cqrs-embedding-gen'}), 200
